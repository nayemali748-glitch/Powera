...
const API_BASE = '/api/google-sheets';
...
async function get(action: string, params: Record<string, string | undefined> = {}) {
  ...
  let res: Response;
  try { res = await fetch(`${API_BASE}?${q.toString()}`, { cache: 'no-store', signal: controller.signal }); } catch (e: any) { throw new Error(e?.name === 'AbortError' ? 'Backend request timed out' : (e?.message || 'Backend connection failed')); } finally { clearTimeout(timer); }
