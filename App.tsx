...
export default function App() {
  ...
  // Backend is the source of truth. Re-check a saved session on every device/app start.
  useEffect(() => {
    if (!currentUser?.idNo) return;
    let cancelled = false;
    verifyUserSession(currentUser.idNo).then(result => {
      if (cancelled) return;
      if (!result.valid) {
      ...
      // Do not silently authenticate a stale local session when backend is unavailable.
      localStorage.removeItem('power_user_session');
      localStorage.removeItem('power_is_admin');
      localStorage.removeItem('power_worker_name');
      if (!cancelled) { setCurrentUser(null); setIsAdmin(false); setWorkerName(''); }
    });
